...
export async function POST(req) {
  try {
    ...
    if (feedback && previousResult) {
      ...
      userText = isHu
        ? `Írj hirdetési szöveget az alábbi óráról. FONTOS: Először keresd meg az órát ezeken a megbízható forrásokon: ${trustedSources}. Keress magyarul ÉS angolul is. Ha ott nem találsz elegendő információt, keress más forrásokban is. A talált adatokat természetesen dolgozd bele a szövegbe.\n\nMárka/modell: ${model}${year ? `\nGyártási év: ${year}` : ""}${caseM ? `\nTok: ${caseM}` : ""}${condition ? `\nÁllapot/megjegyzések: ${condition}` : ""}${image ? "\n\nA képen látható az óra." : ""}`
        : `Write a listing for the following watch. IMPORTANT: First search for information on these trusted sources: ${trustedSources}. Search in both English AND Hungarian. If not enough information is found there, search other sources as well. Naturally weave the found data into the text.\n\nBrand/model: ${model}${year ? `\nYear: ${year}` : ""}${caseM ? `\nCase: ${caseM}` : ""}${condition ? `\nCondition: ${condition}` : ""}${image ? "\n\nThe watch is shown in the image." : ""}`;
    }
